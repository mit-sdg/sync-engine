...
class SignatureParser {
  ...
  #names(role: string): string[] | undefined {
    if (!this.#expect("(", `${role} need an opening "("`)) return undefined;
    if (this.#consume(")")) {
      this.#report(`${role} need at least one field name`);
      return undefined;
    }
    const names: string[] = [];
    while (true) {
      ...
      if (names.includes(field.name)) {
        this.#report(`${role} name "${field.name}" twice`);
        return undefined;
      }
  ...
  parse():
    ...
    if (this.#consumeWord("identified")) {
      if (!this.#consumeWord("by")) {
        this.#report('"identified" must be followed by "by"');
        return undefined;
      }
...
function parseAction(
  ...
  if (signature.identity !== undefined) {
    report(
      "CONCEPT_SPEC_DECLARATION",
      "only a `many` query may declare `identified by (…)`",
      at(group.signature),
    );
    valid = false;
  }
...
function parseQuery(
  ...
  for (const name of signature.identity ?? []) {
    ...
    if (field === undefined) {
      report(
        "CONCEPT_SPEC_DECLARATION",
        `query identity field "${name}" is not one of its result fields`,
        at(group.signature),
      );
      valid = false;
      continue;
    ...
    if (field.optional) {
      report(
        "CONCEPT_SPEC_DECLARATION",
        `query identity field "${name}" cannot be optional`,
        at(group.signature),
      );
      valid = false;
    }
