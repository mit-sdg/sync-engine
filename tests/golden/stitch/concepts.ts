...
export class WorkConcept {
  ...
  add({ title, priority }: { title: string; priority: Priority }) {
    ...
    if (!cleanTitle) return { error: "EMPTY_TITLE", detail: "A title is required" };
  ...
  activate({ id }: { id: string }) {
    ...
    if (!item) return { error: "NOT_FOUND", detail: `No work item named ${id}` };
    if (item.status === "done") return { error: "ALREADY_DONE", detail: `${id} is already done` };
    if (item.status === "active") {
      return { error: "ALREADY_ACTIVE", detail: `${id} is already in focus` };
    }
