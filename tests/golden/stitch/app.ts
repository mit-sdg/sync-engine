...
import { makeStitchFormers, makeStitchReactions } from "./reactions.ts";
...
export async function runCli(args: string[], stateFile: string): Promise<CliResult> {
  ...
  const formers = makeStitchFormers(Work, Focus, History);
  engine.declareFormers(...Object.values(formers));
  ...
  if (command === "status") {
    ...
    if (focus === null || focus.item === null) return success("Nothing in focus.");
    const item = {
      id: focus.item,
      title: focus.title ?? "",
      priority: focus.priority ?? "normal",
