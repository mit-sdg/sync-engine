 ...
 * sync-engine concepts + synchronizations pattern.
 *
 * Concepts:
 ...
 *
 * This file is deliberately self-contained and shows the canonical pattern:
 * concepts own state + actions, syncs compose them declaratively.
 */
...
export class TodoConcept {
  ...
  complete({ id }: CompleteTodoInput) {
    ...
    if (!todo) return { error: "NOT_FOUND", detail: `Todo ${id} not found` };
    todo.completed = true;
    return todo;
  }
