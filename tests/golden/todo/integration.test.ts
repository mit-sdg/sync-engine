...
describe("golden: todo app", () => {
  test("creating a todo creates an audit entry", async () => {
    ...
    // Audit entry was created by the reaction
  ...
  test("completing a missing todo returns an error without an audit entry", async () => {
    const { Todo, Audit } = setup();
    ...
    // The CompleteRecordsAudit reaction requires a successful output (id field),
  ...
  test("multiple actions maintain causal ordering", async () => {
    ...
    // Each mutation has its own flow; reactions fire per-flow.
