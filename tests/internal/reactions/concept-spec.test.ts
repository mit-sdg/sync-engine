...
describe("Queries and canonical compatibility", () => {
  ...
  test("lets a many query promise stable row identity from required result fields", () => {
    const query = validSpecification(
      specification({
        queries:
    ...
    expect(
      ...
      expect.arrayContaining([
        expect.objectContaining({ message: expect.stringContaining("only a `many` query") }),
      ]),
    );
    expect(
      diagnosticsFor(
      ...
      expect.arrayContaining([
        expect.objectContaining({
          message: expect.stringContaining(
            'identity field "missing" is not one of its result fields',
          ),
        }),
      ]),
    );
    expect(
      ...
      expect.arrayContaining([
        expect.objectContaining({
          message: expect.stringContaining('identity field "invitation" cannot be optional'),
        }),
      ]),
    );
  });
