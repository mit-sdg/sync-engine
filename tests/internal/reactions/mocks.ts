...
// Concept whose action resolves in *mutually exclusive* postures: a refusal
// (carrying `error`) or a success carrying `question`. Used to verify that a
// `when` output pattern keyed on a field the record lacks rejects the match.
export class GateConcept {
  public seen: string[] = [];
  check({ value }: { value: number }): { question: string } {
    if (value < 0) throw new Refuse(`negative:${value}`);
...
/** Concept whose action always refuses — a declared refuse branch, spoken via Refuse. */
export class ThrowingConcept {
  public hit = false;
  explode(_: Empty): { error: string; detail?: string } {
    this.hit = true;
    throw new Refuse("KABOOM", { detail: "kaboom" });
  }
  safe(_: Empty) {
    return { ok: true };
  }
...
/** Concept whose action throws a non-Refuse runtime fault. */
export class CrashingConcept {
  public hit = false;
