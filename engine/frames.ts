...
export interface Frames<TFrame extends Frame = Frame> {
   ...
   */
  enrich(
    fn: (frame: TFrame) => Promise<Record<string, unknown>>,
  ): Promise<Frames>;
  ...
  queryOptional(
    ...
    for (const frame of this) {
      ...
      if (rows instanceof Promise) {
        promises.push(
          rows.then((arr) =>
            Frames.expandOptionalOutputs(result, frame, arr, output),
          ),
        );
      } else {
  ...
  bind<TSymbol extends symbol>(
    ...
    const fn =
      typeof valueOrFn === "function"
        ? (valueOrFn as (frame: TFrame) => unknown)
        : undefined;
    for (const frame of this) {
      result.push({
        ...frame,
