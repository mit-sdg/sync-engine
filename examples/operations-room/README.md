...
assembly installs either pack under `compositions` only when its option is
enabled. The discussion pack opens a discussion whose subject is the new
selection. The alert pack raises an alert for every current room member.
`roomDashboard`, installed once under `formers`, joins each discussion and alert
...
declaration. The assembly installs the selected policy under `views` and passes
its complementary permission views to the contribution group; concept
...
| `src/compositions/`                                            | Room endpoints, contributions, and selectable reaction packs            |
| `src/views/`                                                   | Replaceable member-based and host-only contribution policies            |
| `src/formers/Room.ts`                                          | Single owning and installation path for shared room read models         |
| `src/assembly.ts`                                              | Selection and installation under `compositions`, `views`, and `formers` |
