...
test("Profiling principle holds", () => {
  ...
  assert(
    ...
    "Principal lookup changed the display name.",
  );
  ...
  assert(
    stateOf(profiling, profile, principal) === createdState,
    "Duplicate create changed state.",
  );
  ...
  assert(
    profiling._get({ profile })[0]?.displayName === renamedName,
    "Rename changed supplied bytes.",
  );
  assert(
  ...
  try {
    ...
    if (
      ...
      error.message !== "Generated profile identity already exists."
    ) {
      throw error;
    }
  }
  ...
  const invalidDuplicate = new ProfilingConcept(() => invalidDuplicateIDs.shift() ?? "unexpected");
