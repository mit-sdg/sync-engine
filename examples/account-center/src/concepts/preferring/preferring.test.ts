...
test("Preferring principle holds", () => {
  ...
  if (
    !preferring._matches({ owner: "Mina", scope: "workshop", key: "theme", value: "light" }).matches
  ) {
    throw new Error("An exact preference did not match.");
  }
  if (
    preferring._matches({ owner: "Mina", scope: "workshop", key: "theme", value: "dark" }).matches
  ) {
    throw new Error("A different preference value matched.");
  }
  if (
    preferring._matches({ owner: "Mina", scope: "workshop", key: "missing", value: "light" })
      .matches
  ) {
  ...
  if (cleared.preference !== first.preference)
    throw new Error("Clear returned the wrong preference.");
  ...
  try {
    ...
    if (
      ...
      error.message !== "Generated preference identity already exists."
    ) {
      throw error;
    }
  }
