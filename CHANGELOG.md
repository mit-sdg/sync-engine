...
- Added `@mit-sdg/sync-engine-skill` with a compact native-agent workflow for a
  ...
  validation. It depends on the exact matching core, analysis, and catalog packages
  and ships the `sync-engine-skill` brief validator and deterministic file-based prompt
  compiler, with no public JavaScript API or workflow database.
