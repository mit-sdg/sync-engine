...
`inherit` and model `inherit`. For an override authorized by the contract, replace only
the matching model or reasoning control; stop if `invoke_subagent` does not expose it. The child starts
with separate context while editing the same application workspace. Wait once, within the prepared launch deadline, for `Idle`;
