...
`@mit-sdg/sync-engine-http` exposes an assembled application over POST/JSON. Install it
at the exact core version. Its policy derives CORS, request-origin checks, cookie
attributes, and wire projection; authentication and authorization stay in concepts and
composition.
