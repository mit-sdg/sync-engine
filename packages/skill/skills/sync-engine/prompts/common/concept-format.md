...
standalone `()`. Refusal is `refuse CODE "Normative sentence."`.

Action names start with a letter; queries start `_`. Query resolution precedes the
named row: `: one (...)`, `: optional (...)`, or `: many (...)`. Every query has an
indented body covering answer, absence, and deterministic `many` ordering.
