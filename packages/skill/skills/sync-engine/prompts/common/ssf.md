...
inferred named collection; SSF lowercases that exact name's first character. Enums need
...
`plur` pluralizes it or one unique non-element structure or subset to the other exact
spelling. SSF generates no candidate or transitive/third spelling. Exact
declarations win; externals, primitives, elements, and ambiguous candidates get no
automatic alias.

Explicit aliases handle synonyms/ambiguity and override automatic evidence. Targets are
unique valid structures/subsets, never aliases. Parents accept structures, subsets, or
either alias. Normalization rejects unresolved/external/primitive/invalid-alias parents,
duplicate/ambiguous structures, self-parents, and cycles; forward chains work.
...
structural-looking lines fail; standalone non-structural invariants stay opaque and
retained, not proved.
