...
enums uppercase; identifiers use letters/digits/`_`. Collection subjects use
conservative singular/plural pairs, including irregulars. `element` and scalar names are
exact even when ending in `s`. The parser records subsets and inventories owned
identity/type names. Malformed structural-looking lines fail; standalone invariants
remain opaque prose.
