...
contract mismatch as `MATERIAL-NONBLOCKER`. Do not create a report file.

If clean, prove the checks you performed. Return one `CHECK` for every obligation ID in
the accepted map, naming the retry identity, the action that consumes it, and the recovery
source. Then return one `BRIEF` check tracing visible successes, refusals, and scope, and
the exact final verdict. Use only this envelope:

```text
- CHECK `O1` — Retry identity <identity> is consumed by <action>; recovery uses <durable or explicitly ephemeral source>.
- CHECK `BRIEF` — Visible successes and refusals are traced; no material scope mismatch remains.
- VERDICT — No material findings.
```

When any material finding exists, return no checks or verdict; return one bullet per
finding with no preamble:
