...
/** Open discussions and collect responses while they remain open. */
export class DiscussingConcept {
  ...
  open({ subject }: { subject: string }) {
    if (this.#openFor(subject) !== undefined) throw new DiscussionAlreadyOpen();
  ...
  respond({ discussion, author, text }: { discussion: string; author: string; text: string }) {
    ...
    if (found === undefined || !found.open) throw new DiscussionNotOpen();
