...
shapes with resolvable TypeScript source. It also uses exact SSF structural and explicit-alias names to prove
...
bindings, exact SSF-declared definition-owned type inventories, resolved application types,
...
config and form-check tooling separately parses bounded SSF structure and explicit
aliases, validates subset graphs and local namespaces, and inventories exact owned type
names. State field refinements and standalone prose remain open authored contracts;
