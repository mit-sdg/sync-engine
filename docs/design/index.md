...
owns, and how the application connects them. [Define one
behavior](../guide/concepts.md) provides the authoring tutorial and shows how to
...
Each of these follows specifically from the rule that a concept names no peer.
...
- **Every cross-behavior dependency has a name and a location.** "Choosing a
  mitigation opens a discussion" is a declaration in composition. Reviewers can
  enumerate the application's policy by reading composition alone, without
  inspecting `Selecting.choose`.
...
- **Actions commit independently.** An earlier state change remains when a later
  action refuses or faults. See [ordering and state-read
  timing](../semantics.md#ordering-and-state-read-timing).
- **Hosts choose deployment and packaging.** Several concepts can share one
  process and one database; one concept can use several tables. Semantic
  independence remains the same across those arrangements.
- **Storage schemas require an explicit design.** A specification's `State`
  section is uninterpreted human notation. Registration derives no schema,
  validator, or migration from it. See [concept specification
  format](../concept-specification.md#state-notation).
- **Owners establish distributed correctness.** Uniqueness under concurrency,
...
- **A concept needs an actual purpose.** Formal completeness cannot make an
  unnecessary behavior useful.

## Reading path

| Page                                           | Answers                                                                       |
| ---------------------------------------------- | ----------------------------------------------------------------------------- |
| [What a concept is](concepts.md)               | What information defines a concept and which candidates use other boundaries  |
...
| [Reviewing a design](reviewing-a-design.md)    | How to review and revise a whole design across its concepts and composition   |
